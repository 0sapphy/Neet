...
export async function run(interaction: ChatInputCommandInteraction<"cached">) {
    ...
    if (!member) {
        return interaction.reply({
            embeds: [WarningEmbed('Member Not Found', `${emoji('Xmark')} Member not found in this server.`)],
            ephemeral: true
        });
    }
    ...
    if (member.roles.highest.position > interaction.member.roles.highest.position) {
        return interaction.reply({
            embeds: [WarningEmbed("Member Has a Higher Role", `${emoji('Xmark')} ${member.user.username} has a higher role than you (${interaction.user.username})`)],
            ephemeral: true
        });
    }
    ...
    if (!member.kickable) {
        return interaction.reply({
            embeds: [WarningEmbed("Member Is Not Kickable", `${emoji('Xmark')} I can't kick this member.`)],
            ephemeral: true
        });
    }
